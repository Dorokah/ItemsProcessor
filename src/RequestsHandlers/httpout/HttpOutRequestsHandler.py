...
class HttpOutRequestsHandler(RequestHandler):
    ...
    def send_results(self, body, properties, start_timestamp):
        ...
        self.log_received_request(result)
        self.service_logger.add_field('resultUrl', result_url)
    ...
    def log_received_request(self, result):
        self.service_logger.add_field('pipelineName', result['pipelineName'])
        self.service_logger.add_field('entityId', result['entityId'])
        self.service_logger.add_field('requestId', result['requestId'])
        if 'imageFullUrl' in result:
            self.service_logger.add_field('imageFullUrl', result['imageFullUrl'])
        self.service_logger.send_to_logstash("Result pulled")
